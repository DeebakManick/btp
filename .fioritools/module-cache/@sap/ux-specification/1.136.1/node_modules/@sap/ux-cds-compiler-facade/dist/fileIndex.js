...
const processElement = (artifact, collector, sourceUri, source, lines, compiler) => {
    ...
    for (const element of elements) {
        ...
        // process structured types
        processElement(element, collector, sourceUri, source, lines, compiler);
    }
};
...
function createFileIndexFromSource(sourceUri, source, compiler) {
    ...
    const text = source.tokenStream.tokenSource?.inputStream.strdata ?? source.tokenStream.lexer?.input ?? '';
...
function createIdentifierFromToken(uri, token, artifact, buildName) {
    const carrierArtifact = artifact?.name?._artifact ?? artifact;
    const { tokenIndex, text } = token;
    const { line, column } = (0, tokenStream_1.getTokenLocation)(token);
    const definition = artifact?.name && carrierArtifact
        ...
        : undefined;
    return new identifierToken_1.IdentifierTokenImpl(uri, tokenIndex, text, line - 1, column - 1, definition);
}
