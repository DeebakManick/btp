...
function generateMetadataElementFromArtifacts(context, name, artifacts, draftNodesMap, compileModel) {
    ...
    if (['entity', 'view'].includes(topLevelArtifact.kind) &&
        ...
        const isActiveEntityNode = converter.convertDraftPropertyNode(topLevelArtifact, types_1.IS_ACTIVE_ENTITY_ELEM_NAME, 'Edm.Boolean', topLevelArtifactName);
        ...
        const hasActiveEntityNode = converter.convertDraftPropertyNode(topLevelArtifact, types_1.HAS_ACTIVE_ENTITY_ELEM_NAME, 'Edm.Boolean', topLevelArtifactName);
        ...
        const serviceName = converter.getNameObj(topLevelArtifact._service)?.id;
        if (serviceName && compileModel.options.beta?.draftMessages) {
            const draftMessagesNode = converter.convertDraftPropertyNode(topLevelArtifact, types_1.DRAFT_MESSAGES_ELEMENT_NAME, `Collection(${serviceName}.DRAFT_DraftAdministrativeData_DraftMessage)`, topLevelArtifactName);
            if (draftMessagesNode) {
                draftMessagesNode.isCollectionValued = true;
                topLevelArtifactNode.content.push(draftMessagesNode);
                metadataElementMap.set(topLevelArtifactName + '/' + types_1.DRAFT_MESSAGES_ELEMENT_NAME, {
                    parentKey: topLevelArtifactName,
                    node: draftMessagesNode
                });
            }
        }
    }
