...
const typeMap = {
    ...
    'cds.Association': '', // 'Impl.Association',
    'cds.Composition': '', // 'Impl.Composition', relevant for containment?
...
class Converter {
    constructor(compiler, facade) {
        ...
        this.convertElement = (elementName, parentPath, element) => {
            ...
            if (element) {
                ...
                const typeArtifact = element._effectiveType;
                const children = element.items
                    ? element._effectiveType?.items?._effectiveType?.elements
                    : element._effectiveType?.elements;
                const name = this.getNameObj(element.items?.type?._artifact || element._effectiveType);
                if (element.enum) {
                    elementNode.enumValues = this.convertEnums(element.enum);
                }
                if (element.items) {
                    elementNode.isCollectionValued = true;
                }
                elementNode.targetKinds = this.getEdmTargetKinds(elementNode);
                elements.push(...this.collectElementsFromStructuredType(typeArtifact, name, children, elementNode, elementName, parentPath));
        ...
        this.convertParameter = (paramName, parentPath, parameter) => {
            if (parameter) {
                const paramNode = this.buildNode(parameter.kind, paramName, parentPath, this.getOrigin(parameter), this.getPrimitiveType(parameter));
                this.setStructuredType(paramNode, parameter);
                if (parameter.items) {
                    paramNode.isCollectionValued = true;
                }
                if (parameter.enum) {
                    paramNode.enumValues = this.convertEnums(parameter.enum);
                }
        ...
        this.convertDraftPropertyNode = (xsnObject, propName, propertyType, parentPath) => {
            const origin = this.getOrigin(xsnObject);
            origin.name += '.' + propName;
            const draftPropNode = this.buildNode('element', propName, parentPath, origin, propertyType);
    ...
    convertEnums(enums) {
        const result = [];
        for (const [name, enumNode] of Object.entries(enums)) {
            const value = this.convertLiteral(enumNode.value) ?? name; // use name as value if no value is defined
            result.push({
                name,
                value
            });
    ...
    convertLiteral(literalNode) {
        if (!literalNode || !literalNode.literal) {
            return undefined;
        }
        if (literalNode.literal === 'string') {
            return literalNode.val ? literalNode.val.toString() : '';
        }
        else if (literalNode.literal === 'number') {
            return literalNode.val ? Number(literalNode.val) : 0;
        }
        return undefined;
    }
    ...
    collectElementsFromStructuredType(effectiveType, name, children, elementNode, elementName, parentPath) {
        const elements = [];
        if (name && effectiveType && children) {
            // element has structured type
            const { absolute, element } = name;
            elementNode.structuredType = absolute;
            elementNode.isComplexType = true;
            if (effectiveType.kind === 'element' && element) {
                ...
                const flattenedName = element.replace('.', '_');
                elementNode.isAnnotatable = elementNode.path !== `${elementNode.structuredType}/${flattenedName}`;
                elementNode.structuredType += '.' + flattenedName;
            }
            if (effectiveType.items) {
                elementNode.isAnnotatable = true;
                elementNode.isCollectionValued = true;
                return [];
            }
            ...
            for (const [typeElementName, typeElement] of Object.entries(children)) {
    ...
    convertUsingToNamespace(using, fileUri, fileUriRange) {
        return {
            type: 'reference',
            alias: using.name.id, // if no alias is provided, last segment of fully qualified name is used (also aliasRange spans this last segment)
