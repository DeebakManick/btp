...
const flex_1 = require("../../common/decoration/flex");
...
function transferCards(params) {
    Object.keys(params.currentObject).forEach((cardKey) => {
        ...
        if (!Object.values(ux_specification_types_1.v2.CardTemplateType).includes(cardType)) {
            ...
            targetDefinition = Object.keys(params.appSchema.definitions).filter((definition) => {
                const template = params.appSchema.definitions[definition].properties
                    ?.template;
                if (template?.enum) {
                    return template.enum.indexOf(params.currentObject[cardKey].template) > -1;
                }
                else if (template?.const) {
                    return template.const === params.currentObject[cardKey].template;
                }
            });
        }
        //Get metadata
        params.currentObject[cardKey] = assignReflectMetadataRules(params.factory, targetDefinition[0], params.currentObject[cardKey], 'CustomCard');
        createCard(cardKey, params.exportResults.manifest[ux_specification_types_1.ManifestSection.ovp]);
        params.exportResults.manifestChangeIndicator = ux_specification_types_1.ChangeIndicator.Updated;
        transferManifestEntriesOVP(params.currentObject[cardKey], params.exportResults, params.appSchema, params.appSchema.definitions[targetDefinition[0]], params.factory, ['cards', cardKey], params.entityPathArray);
    });
    //check for deletions:
    Object.keys(params.exportResults.manifest[ux_specification_types_1.ManifestSection.ovp].cards).forEach((manifestCard) => {
        if (!params.currentObject[manifestCard]) {
            delete params.exportResults.manifest[ux_specification_types_1.ManifestSection.ovp].cards[manifestCard];
            params.exportResults.manifestChangeIndicator = ux_specification_types_1.ChangeIndicator.Updated;
        }
    });
}
 ...
 */
function exportOfObject(params, propertyKey) {
    if (propertyKey === 'cards') {
        //inserts and updates:
        transferCards(params);
    }
    else {
        const nextTargetDefinition = (0, common_1.getNextTargetDefinition)(params.appSchema, '', params.currentObject, params.propertyList[propertyKey], propertyKey, params.factory, ux_specification_types_1.PageTypeV2.OverviewPage);
        if (params.propertyList[propertyKey].$ref) {
            const definitionArray = params.propertyList[propertyKey].$ref.split(common_1.DEFINITION_LINK_PREFIX);
            params.currentObject = assignReflectMetadataRules(params.factory, propertyKey, params.currentObject, definitionArray[1]);
            params.pathHierarchy = [...params.pathHierarchy, propertyKey];
            transferManifestEntriesOVP(params.currentObject, params.exportResults, params.appSchema, nextTargetDefinition.targetDefinition, params.factory, params.pathHierarchy);
        }
        else if (params.propertyList[propertyKey].anyOf) {
            params.pathHierarchy = [...params.pathHierarchy, propertyKey, 'anyOf'];
            processProperties(params.configObject, params.exportResults, params.appSchema, params.propertyList[propertyKey], params.factory, params.pathHierarchy, params.entityPathArray);
        }
        else if (params.propertyList[propertyKey].properties !== undefined) {
            params.pathHierarchy = [...params.pathHierarchy, propertyKey];
            processProperties(params.configObject[propertyKey], params.exportResults, params.appSchema, params.propertyList[propertyKey].properties, params.factory, params.pathHierarchy, params.entityPathArray);
        }
    }
}
/**
    ...
    for (const propertyKey in propertyList) {
        ...
        if (entityPathArray) {
            if (pathHierarchy &&
                ...
                pathHierarchy[pathHierarchy.length - 1] !== 'anyOf') {
                continue;
            }
        }
        ...
        if (pathHierarchy[pathHierarchy.length - 1] === 'anyOf') {
            ...
            exportOfObject(processDefinitionParams, propertyKey);
        }
...
function evaluateFlexRule(transferParameters, breadcrumbs, syncRule, ids, title, configObject, key) {
    ...
    const completeFlexChange = (0, flex_1.exportToFlexChange)(flexChange, localManifest);
...
function transferColumnExtensions(transferParameters, customColumnsInConfig, schemaDefinition, objectPageSectionData) {
    ...
    for (const extensionType in ux_specification_types_1.v2.TableColumnExtensionTypeV2) {
        ...
        try {
            ...
            transferParameters.logger.error(error.message);
        }
    }
