...
class DistTagEvaluator {
    ...
    doesUi5VersionFitDistTags(minUI5Version, logger) {
        ...
        let allVersions = [];
        if (typeof minUI5Version === 'string') {
            allVersions.push(minUI5Version);
        }
        else if (Array.isArray(minUI5Version)) {
            allVersions = minUI5Version;
        }
        let foundInDistTags = false;
        allVersions.forEach((minUI5Version) => {
            if (!this.checkVersionName(minUI5Version, logger)) {
                return;
            }
            const actualVersion = minUI5Version === 'latest' ? 'latest' : minUI5Version.substring(0, minUI5Version.lastIndexOf('.'));
            if (DistTagEvaluator.distTagsOfSpec.findIndex((distTagVersion) => distTagVersion.endsWith(actualVersion)) >
                -1) {
                foundInDistTags = true;
            }
        });
        if (!foundInDistTags) {
