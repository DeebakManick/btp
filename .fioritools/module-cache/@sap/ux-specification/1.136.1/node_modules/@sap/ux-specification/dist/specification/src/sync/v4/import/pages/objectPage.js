...
/**
 * Fills table actions and custom section actions in config.json.
 *
 * @param {ImportObjectPageV4Parameters} importParameters - object comprising all input data
 * @param {Definition} actionsDefinition - actions definition from schema
 * @param {string} sectionId - ID of the current section
 * @param {object} configPart - the given part of the object page config
 * @param {MetadataInstanceInterface} factory - factory, for generating the access to reflect-metadata
 * @param manifestSectionId - section id in manifest
 */
function fillSectionActionsInConfig(importParameters, actionsDefinition, sectionId, configPart, factory, manifestSectionId) {
    if (typeof actionsDefinition === 'object') {
        for (const actionKey in actionsDefinition['properties']) {
            ...
            if (typeof actionDefinition === 'object' && actionDefinition.$ref) {
                ...
                const definition = actionsDefinition?.['properties']?.[actionKey];
                const schemaPropertyName = definitionKey.split('<')[0];
                let configActionsPart;
                if (schemaPropertyName.includes(ux_specification_types_1.DefinitionName.CustomSectionActionOP)) {
                    // custom section actions
                    configActionsPart = configPart['actions'];
                }
                else {
                    // table toolBar actions
                    configActionsPart = configPart[sectionId].table.toolBar.actions;
                }
                const importActionParams = {
                    actionPropertyDefinition: definition,
                    appSchema: importParameters.jsonSchema,
                    factory,
                    actionsInConfig: configActionsPart,
                    actionKey,
                    manifest: importParameters.manifest,
                    routingId: importParameters.routingId,
                    manifestSectionId
                };
                (0, utils_2.importActionSettingsOfReference)(importActionParams);
            }
        }
    }
}
 ...
 *
 * @param importParameters - object comprising all input data
 * @param manifestSection - section of the manifest.json containing the relevant settings
 * @param sectionId - ID of the current section
 * @param configPart - the given part of the object page config
 * @param factory - factory, for generating the access to reflect-metadata
 * @param manifestSectionId - ID of the section in the manifest
 */
function fillFormActionsInConfig(importParameters, manifestSection, sectionId, configPart, factory, manifestSectionId) {
    if (manifestSection['actions']) {
        const configActionsPart = (configPart[sectionId].form.actions = {});
        ...
        for (const actionKey in actionsDefinition['properties']) {
            ...
            if (formActionDefinition['annotationType'] === "com.sap.vocabularies.UI.v1.DataFieldForAction" /* UIAnnotationTypes.DataFieldForAction */ ||
                formActionDefinition['annotationType'] === "com.sap.vocabularies.UI.v1.DataFieldForActionGroup" /* UIAnnotationTypes.DataFieldForActionGroup */ ||
                formActionDefinition.description === 'Custom Action') {
                const importActionParams = {
                    actionPropertyDefinition: actionDefinition,
                    appSchema: importParameters.jsonSchema,
                    factory,
                    actionsInConfig: configActionsPart,
                    actionKey,
...
/**
 * Retrieves the table definition name based on the provided table type.
 *
 * @param tableType - The type of the table for which the definition name is required.
 * @returns - The corresponding table definition name, or a default name if the table type is not found.
 */
function getTableDefinitionNameByType(tableType) {
    const tableDefinitionsMap = new Map([
        [v4_1.TableTypeV4.AnalyticalTable, ux_specification_types_1.DefinitionName.ObjectPageAnalyticalTableV4],
        [v4_1.TableTypeV4.GridTable, ux_specification_types_1.DefinitionName.ObjectPageGridTableV4],
        [v4_1.TableTypeV4.ResponsiveTable, ux_specification_types_1.DefinitionName.ObjectPageResponsiveTableV4],
        [v4_1.TableTypeV4.TreeTable, ux_specification_types_1.DefinitionName.ObjectPageTreeTableV4]
    ]);
    return tableDefinitionsMap.get(tableType) ?? ux_specification_types_1.DefinitionName.ObjectPageResponsiveTableV4;
}
/**
 ...
 * @param sectionId - ID of the current section
 * @param factory - factory, for generating the access to reflect-metadata
 * @param tableId - ID of the table in the manifest, if available
 */
function fillTableInConfig(importParameters, manifestSettings, configPart, sectionId, factory, tableId) {
    ...
    if (manifestSection) {
        const { tableSettings = {} } = manifestSection;
        const tableType = tableSettings['type'];
        const tableDefinitionName = getTableDefinitionNameByType(tableType);
        // Data needs to be transferred
        configPart[sectionId] = {};
        configPart[sectionId].table = factory.createInstance(ux_specification_types_1.PageTypeV4.ObjectPage, tableDefinitionName);
        const sectionDefinition = `${tableDefinitionName}<${tableId ? getDefinitionId(tableId) : sectionId.replace('#', '::')}>`;
        ...
        fillSectionActionsInConfig(importParameters, actionsDefinition, sectionId, configPart, factory, manifestSectionId);
...
function fillCustomSectionsInConfig(importParameters, manifestSettings, objectPageConfig, factory) {
    ...
    if (customSections) {
        for (const sectionId in customSections) {
            ...
            // fill custom section with custom actions
            if (customSection.actions) {
                section.actions = factory.createInstance(ux_specification_types_1.PageTypeV4.ObjectPage, ux_specification_types_1.DefinitionName.ObjectPageCustomSectionActions, {});
                const actionsDefinition = importParameters.jsonSchema.definitions[`${ux_specification_types_1.DefinitionName.ObjectPageCustomSectionActions}<${getDefinitionId(sectionId)}>`];
                fillSectionActionsInConfig(importParameters, actionsDefinition, sectionId, section, factory, sectionId);
            }
 ...
 * @param routingId - routing ID of the page
 */
 ...
 * @param factory - factory, for generating the access to reflect-metadata
 * @returns the object page configuration
 */
