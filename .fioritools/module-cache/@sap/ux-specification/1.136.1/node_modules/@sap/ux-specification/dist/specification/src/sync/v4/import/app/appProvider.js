...
exports.V4AppProvider = exports.unhandledControllers = exports.Constants = void 0;
 ...
 * @returns {string} Resolved unique target namet.
 */
 ...
 * @returns {object} Map object of association routes.
 */
...
/**
 * Converts the settings of a page to its corresponding configuration.
 *
 * @param params - The parameters required for the conversion, including target, pageType, manifest, schemas, logger, page, id, templateType, and extensionFiles.
 */
 ...
 *
 * @param target - target definition in manifest
 * @param page - page config, to be updated
 */
...
function evaluateCustomPageSettings(target, page, pageType) {
    if ('viewId' in target) {
        ...
        let viewName = target.options?.settings?.viewName || target['viewName'];
        let template = target.name;
        if (!viewName && pageType === ux_specification_types_1.PageTypeV4.CustomPage) {
            // Freestyle page - read 'viewName' from property 'name' if 'viewName' does not exist.
            viewName = template;
            template = undefined;
        }
        page.view = {
...
function getSinglePageAndConfig(routing, id, pages, importParameters) {
    ...
    // Store original page name/template - it is necessary for export as there can be custom templates like "sap.fe.ariba.ListReport"
    if (!page.view?.template && 'name' in target && target.name) {
        page.template = target.name;
    }
    pages[id] = page;
...
function getPages(manifest, targetName) {
    ...
    if (routing) {
        for (const id in routing.targets) {
            ...
            if ((0, common_1.compareTemplateNames)(targetName, v4Page['name'])) {
                pages[id] = v4Page;
            }
        }
    }
...
class V4AppProvider extends appProvider_1.AppProvider {
    ...
    constructor(importParameters) {
        ...
        super(importParameters, getHome(importParameters.manifest[ux_specification_types_1.ManifestSection.ui5].routing), getPagesAndConfigs(importParameters), {
