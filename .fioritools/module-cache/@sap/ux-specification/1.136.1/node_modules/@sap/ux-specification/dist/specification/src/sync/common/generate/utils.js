...
exports.getFacetDefinitionKey = getFacetDefinitionKey;
...
exports.getUniqueFacetDefinitionLink = getUniqueFacetDefinitionLink;
...
function parseSchemaDefinition(definition, facetKey, appSchema, useDefinitionAsKey = true) {
    let newDefinitionKey = facetKey;
    if (useDefinitionAsKey) {
        newDefinitionKey = getFacetDefinitionKey(definition, facetKey);
    }
    if (definition && definition !== newDefinitionKey) {
        try {
            const schemaDefinition = JSON.parse(JSON.stringify(appSchema.definitions[definition]));
            appSchema.definitions[newDefinitionKey] = schemaDefinition;
        }
        catch {
            throw new Error(i18next_1.default.t('INCONSISTENTSCHEMA', { function: 'parseSchemaDefinition', definition }));
        }
    }
    return appSchema.definitions[newDefinitionKey];
}
/**
 ...
 */
function getUniqueFacetDefinitionLink(definition, facetKey, appSchema, lineItemPath) {
    let index = 0;
    let isFacetDefinitionKeyUniqOrReusable = false;
    let facetDefinitionLink = '';
    do {
        // The definition link at index 0 serves as the default one
        const indexedFacetKey = `${facetKey}${index === 0 ? '' : '.' + index}`;
        facetDefinitionLink = getFacetDefinitionLink(definition, indexedFacetKey);
        index++;
        // Check if the facet definition already exists in the application schema
        // If the duplicated element is placed in the same line item, we can reuse the definition
        const facetDefinition = appSchema.definitions[getFacetDefinitionKey(definition, indexedFacetKey)];
        const facetLineItemPath = (facetDefinition?.[`annotationPath`] ?? '').split('/').slice(0, -1).join('/');
        isFacetDefinitionKeyUniqOrReusable = !facetDefinition || facetLineItemPath === lineItemPath;
    } while (!isFacetDefinitionKeyUniqOrReusable);
    return facetDefinitionLink;
}
/**
...
function handleAddressContact(facet, facetKey, sections, appSchema, sectionPrefix, subSectionPrefix, versionPostfix = '') {
    ...
    const definition = parseSchemaDefinition(subSectionPrefix ? subDefinitionName : definitionName, facetKey, appSchema, true);
    (0, __1.addSectionTitle)(facet, definition, facetKey);
    (0, __1.addSectionDescription)(facet, definition, facetKey);
...
function getDataFieldDescription(dataFieldAbstract, entityType) {
    try {
        const strategy = DataFieldStrategy_1.dataFieldStrategyContext.getStrategy(dataFieldAbstract.$Type);
        return strategy.generateDescription(dataFieldAbstract, entityType);
    }
    catch {
        return '';
    }
 ...
 *
 * @param element The schema element to which the message will be added.
 * @param message The message text to add to the schema element.
 */
function addMessageToSchema(element, message) {
