 ...
 */
function dereference(schema, reference, output, definition, description, additionalMetadata, descriptionSrcUrl) {
    const currentDefinition = reference.split('#/definitions/')[1];
    try {
        parseSchema(schema, schema['definitions'][currentDefinition], output, definition, description, additionalMetadata, descriptionSrcUrl);
    }
    catch (error) {
        console.log(error);
    }
 ...
 */
function parseSchema(schema, subSchema, output, definition, description, additionalMetadata, descriptionSrcUrl) {
    ...
    for (const property in subSchema['properties']) {
        if (property === '$ref') {
            dereference(schema, subSchema['properties']['$ref'], output, definition, descriptionSrcUrl);
        }
        else if (property === 'anyOf') {
            handleAnyOf(schema, subSchema['properties']['anyOf'], output, definition, descriptionSrcUrl);
        }
        else {
