 ...
 * @returns a filtered list of facet and line item annotations
 */
function getAnnotationsForUi(oDataServiceAVT) {
    ...
    for (const entity of allEntityTypes) {
        ...
        const { LineItem: lineItem, Facets: facets } = entity.annotations[alias];
 ...
 * @param genericSchemas - The generic schemas
 * @param appSchemas - the list of app schemas that shall get extended
 */
 ...
 * @param generateParameters - list of API input parameters
 * @param genericSchemas - The generic schemas
 * @param uiEntityTypes - list of all entity types, as comprised in oDataServiceAVT
 * @param templateType - the given Fiori Elements' template type
 * @returns The generated app schemas including generic schemas.
 */
 ...
 * @returns {Schemas} - the generated app schemas
 */
...
/**
 * Generates a list of files based on the provided schemas and optional configuration files.
 *
 * @param appSchemas - The schemas to be converted into files.
 * @param configs - Optional configuration files to be included in the output.
 * @returns A list of files containing schema and configuration data.
 */
...
async function importProjectSchema(importProjectParameters) {
    ...
    const fioriElementsVersion = (0, utils_2.getFEVersionFromManifest)(manifest);
...
async function importProjectSchemaAndConfig(importProjectParameters) {
    ...
    versionChecker.doesUi5VersionFitDistTags(manifest['sap.ui5']?.dependencies?.minUI5Version, logger);
    const fioriElementsVersion = (0, utils_2.getFEVersionFromManifest)(manifest);
    const oDataVersion = (0, utils_2.getODataVersionFromManifest)(manifest);
    const oDataServiceAVT = (0, utils_2.parseAndMergeAndConvert)(annotations, logger);
    const templateType = (0, utils_2.getTemplateTypeFromManifest)(manifest, fioriElementsVersion, logger);
    //Generate Schemas
    const generateParameters = {
