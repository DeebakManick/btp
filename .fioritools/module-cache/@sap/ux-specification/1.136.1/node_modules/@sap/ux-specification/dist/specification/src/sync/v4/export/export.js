...
function determineTargetAnnotationOfSections(ids, custom) {
    ...
    let targetAnnotation, targetAnnotationEncoded;
    if (ids[0] === 'sections' && !isAnnotationBased) {
        custom = true;
        targetAnnotationEncoded = ids[1];
        targetAnnotation = targetAnnotationEncoded && targetAnnotationEncoded.replace(/::/g, '/');
    }
    else {
...
function evaluateExportRule(parameters, ids, nextDefinition) {
    ...
    if (syncRule) {
        if (syncRule.xml) {
            ...
            const changed = exportManifestChange(ids, syncRule, parameters, nextDefinition);
            if (changed) {
                parameters.transferParameters.exportResults.manifestChangeIndicator = ux_specification_types_1.ChangeIndicator.Updated;
            }
        }
    }
}
...
function transferSubObject(parameters, nextDefinition, currentConfigObject, ids, localIdParts) {
    ...
    else if (parameters.propertyDefinition['anyOf'] && nextDefinition) {
        objects.push(nextDefinition.configObject);
    }
    ...
    for (const innerObject of objects) {
        ...
        transferManifestEntriesAndFlexChange(parameters.transferParameters, innerObject, [...ids], localIdParts, [...parameters.breadcrumbs, parameters.key], nextDefinition?.targetDefinition);
        parameters.index++;
    }
}
/**
...
function createNewElement(parameters, definitionName, preprocessed) {
    ...
    if (definitionName && !preprocessed && !parameters.transferParameters.deletionRequest) {
...
function transferConfigEntriesWithAnyOfDefinitions(transferEntryParameters, anyOfDefinitions, schemaDefinition, parentIds, stableIdParts) {
    ...
    if (anyOfDefinitions) {
        ...
        configEntries.sort((a, b) => a[1] && a[1]['index'] - b[1]['index']);
...
function getClassesForSingleSection(objectPageConfig, sectionId, factory) {
    ...
    if (customSections && customSections[sectionId]) {
        customSections[sectionId] = factory.createInstance(ux_specification_types_1.PageTypeV4.ObjectPage, 'ObjectPageCustomSectionView', customSections[sectionId], customSections[sectionId]['className']);
    }
}
/**
 ...
 */
const exportCustomPage = (exportParameters) => {
    const { jsonSchema, views, page, manifest, logger } = exportParameters;
    const exportResults = prepareDefaultExportResult(exportParameters.manifest);
    let viewData = undefined;
    if (jsonSchema && Object.keys(jsonSchema).length > 0) {
        viewData = (0, utils_3.getPageCustomViewFile)(views || [], page.name, manifest, exportParameters.logger);
    }
    if (viewData) {
        (0, fpmCustomPage_1.getExtensionExportResult)(viewData, exportResults, page.config, jsonSchema, 'view', logger);
    }
    return exportResults;
};
/**
...
const exportConfigEntityByPathV4 = (exportParametersV4, ui5Version, deletionRequest = false) => {
    if (exportParametersV4[ux_specification_types_1.SchemaType.ListReport]) {
        ...
        return exportCustomPage(exportParametersV4[ux_specification_types_1.SchemaType.FPMCustomPage]);
    }
};
