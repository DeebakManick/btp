...
class Visitor {
    ...
    flattenedAnnotation(astNode, node, pointer) {
        if (node.type !== odata_annotation_core_types_1.ELEMENT_TYPE || (node.name !== "Record" /* Edm.Record */ && node.name !== "Annotation" /* Edm.Annotation */)) {
            return undefined;
        }
        const [segment, indexSegment, ...segments] = pointer;
        if (segment === 'content') {
            ...
            if (!isNaN(index) && segment === 'content') {
                ...
                else if (node.name === "Annotation" /* Edm.Annotation */) {
    ...
    flattenedAnnotationValue(astNode, node, pointer) {
        if (node?.type !== odata_annotation_core_types_1.ELEMENT_TYPE || !astNode.value) {
            return undefined;
        }
        if (pointer.length === 0) {
            return {
